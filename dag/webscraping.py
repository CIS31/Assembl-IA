...
# Fonction pour convertir une date française en datetime
def french_date_to_datetime(date_str):
    ...
    parts = date_str.split('-')  # Ex: ['lundi', '27', 'mai', '2024']
    if len(parts) == 4:
        jour = int(parts[1])
...
# Extraire tous les liens avec leur date convertie en datetime
links_dates = []
...
# Trier les dates par datetime décroissante (les plus récentes d'abord)
links_dates.sort(key=lambda x: x[2], reverse=True)

# Extraire toutes les dates uniques (en string) dans l'ordre décroissant
unique_dates = []
seen = set()
...
# Récupérer tous les liens correspondant à cette deuxième date (sans pdf ni fragment)
matching_links = set()
...
# Prendre le dernier lien (trié alphabétiquement) pour cette date
last_link = sorted(matching_links)[-1]
print("Dernier lien pour la deuxième date :", base_url + last_link)

# Accéder à la page du dernier lien et récupérer le lien vers le fichier XML
...
if xml_response.status_code == 200:
    ...
    # === MODIF ICI ===
    # Sauvegarder dans text/input à partir de ce fichier situé dans dag/
    current_dir = Path(__file__).resolve().parent      # .../dag
    input_dir = current_dir.parent / "text" / "input"  # .../text/input
    input_dir.mkdir(parents=True, exist_ok=True)       # crée le dossier si besoin

    file_path = input_dir / filename

    with open(file_path, "wb") as f:
        f.write(xml_response.content)
    print(f"Fichier XML téléchargé avec succès : {file_path}")
else:
